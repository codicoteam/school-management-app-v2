...
const StudentFeesPage = () => {
  ...
  const handlePayment = () => {
    ...
    setPayOpen(false);
    alert(`Payment of $${paymentAmount} successful! Receipt: ${newReceipt.id}`);
  };

  const downloadReceipt = (receipt: typeof history[0]) => {
  ...
  const newPaid = paid + (history[0]?.amount || 0);
          ...
          <DialogContent>
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <p className="text-xs text-muted-foreground">Balance due: ${balance}</p>
              </div>
              <div className="grid gap-2">
            ...
            <DialogFooter><Button variant="outline" onClick={() => setPayOpen(false)}>Cancel</Button><Button onClick={handlePayment}>Pay ${paymentAmount}</Button></DialogFooter>
          </DialogContent>
        </Dialog>
      </motion.div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
          ...
          <CardContent className="relative p-6">
            ...
            <div className="mt-4">
              <div className="mb-1 flex justify-between text-xs text-muted-foreground">
                <span>Paid: ${newPaid}</span><span>Total: ${total}</span>
              </div>
              <Progress value={(newPaid / total) * 100} className="h-2" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-md">
          ...
          <CardContent className="space-y-2">
            {breakdown.map(b => (
              <div key={b.item} className="flex justify-between text-sm"><span className="text-muted-foreground">{b.item}</span><span className="font-medium">${b.amount}</span></div>
            ))}
        ...
        {[
          ...
          <Card key={s.label} className="border-none shadow-md">
            <CardContent className="flex items-center gap-3 p-4">
              ...
              <div><p className="text-xs text-muted-foreground">{s.label}</p><p className="text-sm font-semibold">{s.text}</p></div>
            </CardContent>
          </Card>
        ))}
      </div>
