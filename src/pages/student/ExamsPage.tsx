      ...
      <Tabs defaultValue="results" className="space-y-4">
        ...
        <TabsContent value="results">
          <Card className="border-none shadow-md">
            <CardHeader><CardTitle className="font-heading text-lg font-semibold">Term 1 — 2025 Results</CardTitle></CardHeader>
            ...
            <CardContent className="p-4 lg:p-6 space-y-3">
              {upcomingExams.map((e, i) => (
                <div key={i} className="flex items-center gap-3 rounded-lg bg-muted/40 p-3">
                  <div className={`h-10 w-1 rounded-full ${e.color}`} />
                  <div className="flex-1 min-w-0">
                    <p className="font-medium">{e.name}</p>
                    <p className="text-xs text-muted-foreground">{e.date} · {e.time} · {e.venue}</p>
                  </div>
