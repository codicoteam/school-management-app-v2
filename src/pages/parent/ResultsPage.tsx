  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Button variant="outline"><Download className="h-4 w-4" /> Download Report Card</Button>
      </motion.div>
      ...
      <Tabs defaultValue="subjects" className="space-y-4">
                ...
                <div key={r.subject}>
                  <div className="mb-1.5 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">Teacher: {r.teacher}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <Badge className="bg-accent/15 text-accent hover:bg-accent/20">{r.grade}</Badge>
                      <span className="w-12 text-right text-sm font-semibold">{r.score}%</span>
                    </div>
                  </div>
        ...
        <TabsContent value="trend">
          <Card className="border-none shadow-md">
            <CardHeader><CardTitle className="font-heading text-lg font-semibold">Average over time</CardTitle></CardHeader>
            <CardContent>
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                {trend.map((t) => (
                  <div key={t.term} className="rounded-lg bg-muted/40 p-3">
                    <p className="text-xs text-muted-foreground">{t.term}</p>
                    <p className="font-bold">{t.avg}% <span className="text-xs font-normal text-muted-foreground">· #{t.position}</span></p>
                  </div>
                ))}
              </div>
            </CardContent>
