    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Button className="bg-accent text-accent-foreground hover:bg-accent/90"><Save className="h-4 w-4" /> Save Attendance</Button>
        </div>
      </motion.div>
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: "Class Avg", value: "92%", icon: Clock, color: "from-accent to-accent/70" },
        ].map((s) => (
                        ...
                        <div className="inline-flex gap-1">
                          {(["Present", "Late", "Absent"] as Status[]).map((st) => (
                            <Button
                              ...
                              className={s.status === st ? (
                                st === "Present" ? "bg-green-600 hover:bg-green-700" :
                                st === "Late" ? "bg-orange-500 hover:bg-orange-600" :
